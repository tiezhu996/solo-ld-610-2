...
export const restorationPlanController = {
  ...
  reject: asyncHandler((req: AuthedRequest, res: Response) => {
    ...
    res.json(transferLoopService.rejectPlan(req.params.id, { reason: body.reason }, actor, crashPhase));
  }),
  ...
  archive: asyncHandler((req: AuthedRequest, res: Response) => {
    const actor = req.user ?? { id: 0, role: "anonymous" };
    const crashPhase = parseCrashPhase(req.header("x-crash-phase"));
    res.json(transferLoopService.archivePlan(req.params.id, actor, crashPhase));
  }),
};
